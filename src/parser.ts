...
async function* parseChunk(chunks: AsyncGenerator<ArrayBuffer>, debug = false): AsyncGenerator<OsmBlock> {
...
function decodeBlock(block: any) {
    ...
    const getLat = lat => .000000001 * (lat_offset + (granularity * lat));
    const getLon = lon => .000000001 * (lon_offset + (granularity * lon))
